    ...
    public void Extract()
    {
        ...
        for (int i = 0; i < terrain.terrainData.treePrototypes.Length; i++)
        {
            ...
            for (int j = 0; j < instances.Length; j++)
            {
                ...
                obj.transform.localScale = new Vector3(instances[j].widthScale, instances[j].heightScale, instances[j].widthScale);
                obj.transform.localRotation = Quaternion.Euler(0, Mathf.Rad2Deg * instances[j].rotation, 0);
                obj.isStatic = true;
            }
        }
    }
